...
F2s = linspace(0,max_f2,20);

% climate velocities
Vs = linspace(1,max_m2,20);
...
for g = 1:length(G12s)
    ...
    for f2 = 1:length(F2s)
    for v = 1:length(Vs)
        [P1(f2,v,m),P2(f2,v,m),dN(f2,v,m)] = unstruct2sp(Vs(v),F2s(f2),Rw(m),Pr(m),G12s(g),G21s(g),F1s(f),0);
...
unstruct2sp(Vs(2),F2s(12),Rw(1),Pr(1),G12s(1),G21s(1),F1s(2),1);
% high fishing, habitat expanding - persistence
unstruct2sp(Vs(4),F2s(12),Rw(1),Pr(1),G12s(1),G21s(1),F1s(2),1);

% high fishing, slow climate + small MPAs - persistence
unstruct2sp(Vs(2),F2s(12),Rw(2),Pr(2),G12s(1),G21s(1),F1s(2),1);

% high fishing, slow climate + large MPAs - no persistence
unstruct2sp(Vs(2),F2s(12),Rw(3),Pr(3),G12s(1),G21s(1),F1s(2),1);  
